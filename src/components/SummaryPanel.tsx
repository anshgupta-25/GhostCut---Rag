...
function EvidenceNode({
  ...
  return (
    <div className={cn("ml-10 border-l-2 border-border pl-4 py-2 animate-fade-in", section.importance && importanceBorderStyle[section.importance.level])} data-search-id={`summary-${section.id}-0`}>
      ...
      <p className="text-xs text-secondary-foreground leading-relaxed">{Array.isArray(section.summary) ? section.summary.join(" ") : section.summary}</p>
      {section.evidence && (
        <div className="mt-2 rounded-lg bg-muted/50 p-2.5 text-xs font-mono text-muted-foreground leading-relaxed border border-border">
          ...
          {Array.isArray(section.evidence) ? section.evidence.join(" ") : section.evidence}
        </div>
      )}
      ...
      <div className={cn(
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-secondary-foreground leading-relaxed">{Array.isArray(section.summary) ? section.summary.join(" ") : section.summary}</p>
    ...
    <div className="animate-fade-in space-y-3" data-search-id={`summary-${section.id}-0`}>
      <div className={cn(
        ...
        <div className="space-y-1.5">
          {(Array.isArray(section.summary) ? section.summary : String(section.summary || "").split(/\n|(?:^|\s)[-•]\s/)).filter(Boolean).map((point, i) => (
