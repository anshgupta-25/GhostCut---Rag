...
function extractTextFromSections(sections: SummarySection[], results: SearchMatch[], terms: string[]) {
  for (const section of sections) {
    for (let ti = 0; ti < terms.length; ti++) {
      const term = terms[ti];
      const summaryText = Array.isArray(section.summary) ? section.summary.join(" ") : String(section.summary || "");
      if (summaryText.toLowerCase().includes(term)) {
        results.push({
    ...
    if (section.evidence) {
      const evidenceText = Array.isArray(section.evidence) ? section.evidence.join(" ") : String(section.evidence || "");
      for (let ti = 0; ti < terms.length; ti++) {
        if (evidenceText.toLowerCase().includes(terms[ti])) {
          results.push({
...
function buildSuggestions(
  ...
  const allText = [
    ...summaries.map(s => Array.isArray(s.summary) ? s.summary.join(" ") : String(s.summary || "")),
    ...summaries.flatMap(s => s.evidence ? [Array.isArray(s.evidence) ? s.evidence.join(" ") : String(s.evidence)] : []),
    ...chunks.map(c => c.text),
