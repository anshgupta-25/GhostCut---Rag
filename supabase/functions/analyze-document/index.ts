...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "compress") {
      return await handleCompress(text, fileName, OPENAI_API_KEY);
    } else if (action === "audit") {
      return await handleAudit(query, text, chunks, OPENAI_API_KEY);
    }
...
async function callAI(messages: any[], OPENAI_API_KEY: string, tools?: any[], tool_choice?: any) {
  const body: any = {
    model: "gpt-4o-mini",
    messages,
  };
